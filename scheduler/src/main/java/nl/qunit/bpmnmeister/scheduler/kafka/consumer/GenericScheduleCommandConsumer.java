...
/**
 * Consumes schedule command messages from a Kafka topic, deriving classes call a scheduler to
 * schedule them and cancel the commands if tombstone messages arrive or the partition gets revoked
 * during a re-balancing process.
 *
 * <p>The consumer will always start at offset zero to make sure that all tasks in the assigned
 * partition are handled.
 *
 * <p>This class only works with compacted topics.
 */
public abstract class GenericScheduleCommandConsumer<T extends ScheduleCommand>
    implements KafkaConsumerRebalanceListener, CommandCompletionHandler {

  protected final HashMap<Integer, HashMap<String, String>>
      scheduledCommandsByPartitionByCommandId = new HashMap<>();
  private final HashMap<Integer, HashMap<String, ConsumerRecord<String, T>>>
      pendingConsumerRecordsByPartitionByKey = new HashMap<>();
  private final Logger log = org.slf4j.LoggerFactory.getLogger(getClass());
  private Map<Integer, Long> latestOffsetsByPartition;

  abstract String schedule(ConsumerRecord<String, T> consumerRecord);

  abstract void cancel(String id);

  /**
   * Should be overridden and annotated with @Incoming in the extending class. Overriding
   * implementation should call super.receive(ConsumerRecord<String, byte[]> record);
   *
   * @param consumerRecord the ScheduleCommand identified by a unique KafkaKey
   */
  public void receive(ConsumerRecord<String, T> consumerRecord) {
    log.debug(
        "Received message. Topic: {}, partition: {}, record offset: {}",
  ...
  /**
   * If this partition is caught up schedule the command. If not store the command in the hashmap.
   ...
   */
  private void handleRecord(ConsumerRecord<String, T> consumerRecord) {
    ...
    if (isPartitionCaughtUp(consumerRecord)) {
      ...
      this.pendingConsumerRecordsByPartitionByKey
          .get(consumerRecord.partition())
          .put(consumerRecord.key(), consumerRecord);
    }
  }
  ...
  /**
   * If partition for this consumerRecord is caught up we have already scheduled the command. So we
   * have to retrieve it from the hashmap and cancel it. If the partition is not caught up just
   * remove the pending schedule command from the map, so it won't get scheduled.
   *
   * @param consumerRecord the ScheduleCommand identified by a unique KafkaKey
   */
  private void cancelScheduledCommand(ConsumerRecord<String, ?> consumerRecord) {
    log.debug("Handling cancellation");
    if (pendingConsumerRecordsByPartitionByKey
        .get(consumerRecord.partition())
        .containsKey(consumerRecord.key())) {
      log.info("Unscheduled command was already cancelled: {}", consumerRecord.key());
      pendingConsumerRecordsByPartitionByKey
          .get(consumerRecord.partition())
          .remove(consumerRecord.key());
    } else {
      log.info("Scheduled command should be cancelled: {}", consumerRecord.key());
      if (scheduledCommandsByPartitionByCommandId
          .get(consumerRecord.partition())
          .containsKey(consumerRecord.key())) {
        var tasksForPartition =
            scheduledCommandsByPartitionByCommandId.get(consumerRecord.partition());
        ...
        log.info(
            "There is no command with the id: {} which could be cancelled. Maybe the topic was not "
                + "compacted fast enough",
            consumerRecord.key());
      }
    }
    if (isPartitionCaughtUp(consumerRecord)) {
  ...
  /**
   * Store the endOffset. "How many messages are in the partition at this moment?" Given this
   * information we can compute when this consumer is caught up, has seen all messages and can start
   * scheduling tasks. Without knowing that information we might see schedule commands which will be
   * cancelled a few messages later, so we should not schedule them at all. This can happen when the
   * topic is not compacted fast enough.
   *
  ...
  private void storeLatestOffsets(Consumer<?, ?> consumer, Collection<TopicPartition> partitions) {
    this.latestOffsetsByPartition =
        consumer.endOffsets(partitions).entrySet().stream()
            .collect(
                Collectors.toMap(
  ...
  private void cancelScheduledCommandsForPartition(Integer partition) {
    for (var id : this.scheduledCommandsByPartitionByCommandId.get(partition).keySet()) {
      ...
      cancel(scheduledId);
      this.scheduledCommandsByPartitionByCommandId.get(partition).remove(id);
      log.info("Cancelled command with id: {}.", id);
    }
  }

  /**
   * Check if the offset of the received consumerRecord is at least the same as the latest offset of
   * the partition of this consumerRecord at the time of this consumers start up. The offset has to
   * be increased by one because if the partition contains one consumerRecord its offset will be at
   * 1 meaning "after the `0` offset". But the consumerRecord itself is at offset 0.
   *
   * @param consumerRecord The received Record
   * @return True if the offset of the received consumerRecord is at least the same as the latest
   *     offset of the partition of this * consumerRecord at the time of this consumers start up.
   */
  private boolean isPartitionCaughtUp(ConsumerRecord<String, ?> consumerRecord) {
    return consumerRecord.offset() + 1
        >= this.latestOffsetsByPartition.get(consumerRecord.partition());
  }

  /**
   * Callback to remove the ScheduleCommand from the map of scheduled task to prevent it growing
   * without bounds.
   *
