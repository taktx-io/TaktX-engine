...
/**
 * Fault-tolerant protobuf deserializer with optional engine Ed25519 signature verification.
 *
 * <p>The body is always decoded first. Signature verification is then attempted separately. If it
 * fails the decoded body is still returned inside {@link DeserializationResult} so the consumer can
 * inspect identifiers and report the failure back to the engine.
 *
 * <p>Only body-decode failures (corrupt protobuf bytes / wrong message type) yield {@link
 * DeserializationResult#failure(String)} with a {@code null} value.
 */
@Getter
public abstract class FaultTolerantProtoDeserializer<T extends MessageLite>
  ...
  /** Local consumer property that enables strict inbound signature enforcement. */
  public static final String SIGNING_REQUIRED_CONFIG = "taktx.security.signing.enabled";

  private static final Logger log = LoggerFactory.getLogger(FaultTolerantProtoDeserializer.class);
  ...
  protected FaultTolerantProtoDeserializer(Class<T> clazz, boolean shouldValidateSignature) {
    this.clazz = clazz;
  ...
  private DeserializationResult<T> decodeBody(byte[] data) {
    if (data == null) {
      String msg = "Failed to decode body as " + clazz.getSimpleName() + ": payload is null";
      log.error(msg);
  ...
  /** Returns {@code null} on success, or an error message string on failure. */
