...
class InstanceUpdateRecordEventCheckerTest {
  ...
  void testPublishInstanceUpdateRecord() {
    ...
    InstanceUpdateRecord instanceUpdateRecord =
        new InstanceUpdateRecord(100L, UUID.randomUUID(), null, 1, 200L);

    // When
    checker.publishInstanceUpdateRecord(instanceUpdateRecord);

    // Then
    verify(eventPublisher).publishEvent(instanceUpdateRecord);
  }
