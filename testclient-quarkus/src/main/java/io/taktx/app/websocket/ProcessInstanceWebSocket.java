...
public class ProcessInstanceWebSocket {
    ...
    private final Map<String, CopyOnWriteArrayList<Map<String, Object>>> processInstanceUpdates = new ConcurrentHashMap<>();
    ...
    @OnOpen
    public void onOpen(Session session, @PathParam("processDefinitionId") String processDefinitionId) {
        sessions.put(session.getId(), session);
        sessionToProcessDefinitionId.put(session.getId(), processDefinitionId);
        log.info("WebSocket session opened for process definition {}: {}", processDefinitionId, session.getId());
        ...
        CopyOnWriteArrayList<Map<String, Object>> instances = processInstanceUpdates.get(processDefinitionId);
    ...
    public void onClose(Session session) {
        ...
        log.info("WebSocket session closed for process definition {}: {}", processDefinitionId, session.getId());
    }
    
    @OnError
    public void onError(Session session, Throwable throwable) {
        String processDefinitionId = sessionToProcessDefinitionId.get(session.getId());
        log.error("WebSocket error for session {} (process definition {}): {}", 
                session.getId(), processDefinitionId, throwable.getMessage(), throwable);
    ...
    private void handleInstanceUpdate(UUID processInstanceKey, InstanceUpdateDTO update) {
        ...
        Map<String, Object> instanceUpdate = Map.of(
