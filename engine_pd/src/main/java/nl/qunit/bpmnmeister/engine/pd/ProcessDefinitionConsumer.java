...
public class ProcessDefinitionConsumer {
  ...
  public OutgoingKafkaRecord<String, ProcessDefinition> consumeXml(ConsumerRecord<String, String> xmlRecord) {
    LOG.info("Received process definition XML: " + xmlRecord.value());
    try {
      Definitions definitions = bpmnParser.parse(xmlRecord.value());

      Map<String, ProcessDefinition> hashCodeMap = processDefinitions.computeIfAbsent(xmlRecord.key(), k -> new HashMap<>());
      if (!hashCodeMap.containsKey(definitions.getHash())) {
        ProcessDefinition processDefinition = ProcessDefinition.builder().definitions(definitions).version(hashCodeMap.size() + 1).build();
  ...
  public void consumeParsed(ConsumerRecord<String, ProcessDefinition> processDefinitionRecord) {
    LOG.info("Received process definition record: " + processDefinitionRecord.key() + " " + processDefinitionRecord.value());
    ProcessDefinition previous = processDefinitions.computeIfAbsent(processDefinitionRecord.key(), k -> new HashMap<>()).put(processDefinitionRecord.value().getDefinitions().getHash(), processDefinitionRecord.value());
    if (previous != null) {
      LOG.info("Process definition already exists");
    }
  }
